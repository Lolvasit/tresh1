...
from aiogram.types import InputFile, Message, InlineKeyboardMarkup, InlineKeyboardButton, CallbackQuery, ContentType, \
    BotCommand, ReplyKeyboardMarkup, KeyboardButton, MessageEntity
...
async def menu_msg(num, message, state, call_user_id):
    ...
    try:
        ...
        ctitle=""
        if(user.id in users_verifying):
            ctitle = (await bot.get_chat(users_verifying[user.id])).title

        send_to = user.id

        messagef: Message = json.loads(setting.start_msg)
        text = str(messagef["text"] if "text" in messagef else messagef["caption"]),
        msg = text[0].format(
        ...
        if "entities" in messagef:
            for item in messagef["entities"]:
            ...
            for item in messagef["caption_entities"]:
                entities.append(MessageEntity(
                    item["type"],
                    item["offset"],
                    item["length"],
        ...
        await bot.send_message(send_to, msg, reply_markup=start_kb, entities=entities) if (
                    setting.start_msg_id == 0) else await bot.copy_message(
            send_to,
...
async def _confirm_make_mail(message: Message, state: FSMContext):
    ...
    setting.start_msg = message.as_json()
    setting.save()
    await StartMailingStates.menu.set()
...
async def _process_change_kb_end(message: Message, state: FSMContext):
    ...
    try:
        ...
        ctitle=""
        if(user_id in users_verifying):
            ctitle = (await bot.get_chat(users_verifying[user_id])).title
        ...
        await bot.send_message(send_to, msg, reply_markup=kb, entities=entities) if (
                    setting.start_msg_id == 0) else await bot.copy_message(
            send_to,
            setting.start_from_user_id,
            setting.start_msg_id,
...
async def send_start_msg(send_to, setting_num):
    ...
    try:
        ...
        ctitle=""
        if(send_to in users_verifying):
            ctitle = (await bot.get_chat(users_verifying[send_to])).title
        ...
        semd_msg = await bot.send_message( send_to, msg, reply_markup=start_kb, entities= entities) if (setting.start_msg_id == 0) else await bot.copy_message(
                send_to,
                setting.start_from_user_id,
                setting.start_msg_id,
                caption=msg,
